...
import {
  AL_SADR_SYSTEM_INSTRUCTION_AR,
  AL_SADR_SYSTEM_INSTRUCTION_EN,
  THINKING_BUDGET_PRO,
  THINKING_BUDGET_FLASH,
  GEMINI_2_5_PRO,
} from '../components/constants.jsx'; // Corrected path
...
class GeminiService {
  ...
  async generateContent(
    ...
    const systemInstruction = language === 'ar' ? AL_SADR_SYSTEM_INSTRUCTION_AR : AL_SADR_SYSTEM_INSTRUCTION_EN;
    const config = {
      systemInstruction: systemInstruction,
    };

    if (useThinkingBudget) {
      const budget = model === GEMINI_2_5_PRO ? THINKING_BUDGET_PRO : THINKING_BUDGET_FLASH;
  ...
  async startLiveSession(
    onTranscription, // Callback to update current live transcription display
    ...
    onTurnComplete,   // Callback for when a full turn (input+output) is complete
    language = 'ar'
  ) {
    // Re-initialize GoogleGenAI to ensure latest API key if selected via openSelectKey()
    this.ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      ...
      callbacks: {
        ...
        onmessage: async (message) => {
          ...
          if (message.serverContent?.turnComplete) {
            // Call the onTurnComplete callback to integrate into chat history
            onTurnComplete(currentInputTranscription, currentOutputTranscription);
            // IMPORTANT: internal transcriptions are cleared *after* onTurnComplete is processed by ChatInterface
            currentInputTranscription = '';
            currentOutputTranscription = '';
          }
